...
for imageSource in imageSources:
    ...
    plt.imshow(img)
    #plt.show()
    for i in range(0, len(eyeCollection)):
        plt.imshow(eyeCollection[i])
        plt.savefig(join(imageOutputPath, imageSource) + "_" + str(i) + "_sep.jpg", bbox_inches='tight')
    #plt.show()
    cv2.imwrite(join(imageOutputPath, imageSource) + ".jpg", img)
